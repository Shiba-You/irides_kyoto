  ...
  def make_histgran(self):
    ...
    for i in range(self.f_len):
      if i%2==0:
        ...
        f, axs = plt.subplots(1, 2, figsize=(16, 8))
        plt.subplots_adjust(wspace=0.4, hspace=0.8, bottom=0.17, top=0.93)
      x_range = (min(self.feature[:,i]), max(self.feature[:,i]))
      axs[i%2].hist(feature_A[:, i], range=x_range, bins=10, alpha=0.5, color="red", ec="darkred", label="A")
      axs[i%2].hist(feature_B[:, i], range=x_range, bins=10, alpha=0.5, color="blue", ec="darkblue", label="B")
      axs[i%2].grid()
